...
2. **Для каждого фильма:**
   ...
   - Проверить наличие прав для региона TJ
   - Определить причину недоступности (если есть)
3. **Обновить данные:**
   ```sql
   -- Если недоступен для TJ или ошибка API
...
- ❌ Истекли права - установить `right_end_date` из API
- ⚠️ Ошибка API - установить `right_end_date = вчера`
...
3. **Обновить базу:**
   ...
   AND trailer_not_found_in_api = TRUE;
   ```
...
| `info_text` | TEXT | Описание контента | |
| `created_at` | TIMESTAMP | Дата создания | |
...
1. **Немедленно:** Записать ошибку в лог
2. **Через 5 минут:** Повторить попытку
3. **Через 30 минут:** Отправить уведомление администратору
4. **Через 2 часа:** Запустить ограниченную проверку (только критичный контент)
...
-  Убедиться в отправке уведомлений (если настроены)
