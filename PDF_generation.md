# Тех. задание на сервис генерации PDF-отчётов по приборам учёта
...
Система должна обеспечивать формирование отчёта по всем приборам СНТ, генерация за предыдущий месяц.

### Входные параметры

| Параметр | Тип           | Обязательный | Описание                                     |
| -------- | ------------- | ------------ | ---------------------------------------------|
| snt_id   | integer       | Да           | Идентификатор группы                         |
| offset   | integer       | Нет          | Смещение временной зоны (количество месяцев) |
...
| TARIFF_DIFFERENT      | Тариф менялся в течении выбранного периода |
| CONSUMPTION_NOT_FOUND | Нет данных за период                       |
...
* Внедрения брокера сообщений Kafka для обработки пакетной генерации квитанций
* Настройка системы оповещений о готовности генерации
