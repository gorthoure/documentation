# Документация: Аутентификация на портале через обращение к внутреннему микросервису по API и аутентификация на сервисе поставщика
...
Система обеспечивает аутентификацию пользователей на портале с интеграцией со сторонним сервисом Амедиатека. 
...
- Автоматическая регистрация в сервисе Амедиатеки при первой авторизации.  
...
3. Верификация пользователя через внутренний микросервис по API.  
4. Проверка черного списка.  
5. Проверка корректности пароля.  
6. Проверка подписки и блокировок.  
7. Регистрация пользователя в базе Амедиатеки при необходимости.  
8. Аутентификация на сервисе поставщика (Амедиатека).  
...
graph TD
    ...
    D --> E[Верификация пользователя через внешний микросервис]
    ...
    N -->|Да| P[Поиск пользователя в БД портала]
    P --> Q{Есть регистрация в Амедиатеке}
    Q --> |Да| R[Авторизация в Амедиатеке]
    Q --> |Нет| Z[Регистрация в Амедиатеке]
    R --> S[Аутентификация]
    Z --> S[Аутентификация]
    I --> T[Вернуть ответ на ообработанную ошибку]
    O --> T
    L --> T
    G --> T
    C --> T
